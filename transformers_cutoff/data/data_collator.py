...
from .datasets.glue import InputFeatures
...
class DefaultDataCollator(DataCollator):
    ...
    def collate_batch(self, features: List[InputDataClass]) -> Dict[str, torch.Tensor]:
        ...
        for k, v in vars(first).items():
            if k not in ("label", "label_ids", "example_index") and v is not None and not isinstance(v, str):
...
@dataclass
class DataCollatorForGlueLanguageModeling(DefaultDataCollator):
    tokenizer: PreTrainedTokenizer
    mlm: bool = True
    mlm_probability: float = 0.15
    label_as_seg_ids: bool = False
    ...
    def collate_batch(self, features: List[InputFeatures]) -> Dict[str, torch.Tensor]:
        ...
        if self.label_as_seg_ids:
            new_batch['token_type_ids'] = batch['token_type_ids']
